...
# 2. 실시간 환율 호출 함수 (yfinance 대체)
def fetch_realtime_usd_krw():
    """공개 API를 통해 지연 없는 실시간 환율 데이터 수집"""
    try:
        # Frankfurter API 또는 기타 신뢰할 수 있는 공개 금융 API 사용
        url = "https://api.frankfurter.app/latest?from=USD&to=KRW"
        response = requests.get(url, timeout=5)
        data = response.json()
        return float(data['rates']['KRW'])
    except:
        # API 장애 시 yfinance를 백업으로 사용하거나 기본값 유지
        try:
            ticker = yf.Ticker("USDKRW=X")
            return float(ticker.fast_info['last_price'])
        except:
            return 1380.0 # 최후의 보루
...
def load_and_fix_data():
    try:
        ...
        if holdings.empty: holdings = pd.DataFrame(columns=['시장', '구분', '종목', '티커', '수량', '현재가'])
        if trade_log.empty: trade_log = pd.DataFrame(columns=['Date', 'Ticker', 'Action', 'Quantity', 'Buy_Price', 'Sell_Price', 'Fees', 'Currency', 'Realized_Profit', 'Note'])
        if 'Date' in trade_log.columns: trade_log['Date'] = pd.to_datetime(trade_log['Date'], errors='coerce').dt.date
        for col in ['Quantity', 'Buy_Price', 'Sell_Price', 'Fees', 'Realized_Profit']:
            if col in trade_log.columns: trade_log[col] = pd.to_numeric(trade_log[col], errors='coerce').fillna(0.0)
        return holdings, trade_log
    except Exception as e:
        st.error(f"DB 로딩 오류: {e}"); return pd.DataFrame(), pd.DataFrame()

holdings_df, trade_log_df = load_and_fix_data()

# 4. 사이드바: 🎛️ CONTROL PANEL & 실시간 환율 연동
if 'usd_krw' not in st.session_state:
    st.session_state['usd_krw'] = 1490.0 # 2026-05-14 기준 초기값
...
with st.sidebar:
    ...
    edited_holdings = st.data_editor(holdings_df, num_rows="dynamic", key="holdings_editor", use_container_width=True,
        column_config={"시장": st.column_config.SelectboxColumn("시장", options=["해외", "국내"]),
                       "구분": st.column_config.SelectboxColumn("구분", options=["Leverage", "Dividend", "예수현금"]),
                       "종목": st.column_config.TextColumn("종목"), "티커": st.column_config.TextColumn("티커"),
                       "수량": st.column_config.NumberColumn("수량", min_value=0.0), "현재가": st.column_config.NumberColumn("현재가", min_value=0.0)})
...
# 5. 코어 엔진 로직
def run_shabal_v23_logic(df, cash_krw, rate):
...
total_asset_krw, cur_ratio, drift_lev = run_shabal_v23_logic(edited_holdings, monthly_cash, usd_krw_live)


# =====================================================================
# [하편] 시각화 대시보드 및 정렬된 Serial Portfolio Analysis
# =====================================================================

st.title("⚡ SHABAL: SHANNON'S REBALANCING")

col_m1, col_m2 = st.columns([1.5, 1])
with col_m1: goal_amt = st.number_input("🎯 2031 목표 확보액 (KRW)", value=250000000, step=10000000)
with col_m2: st.metric("D-DAY TO 2031", f"{(date(2031,12,31)-date.today()).days} Days")
...
with t1:
    ...
    fig_gauge = go.Figure(go.Indicator(mode="gauge+number", value=cur_ratio, number={'suffix': "%", 'font': {'color': '#00F3FF'}},
...
# 7. Serial Portfolio Analysis (최종 금액 기준 정렬 & 벤치마크 포함)
st.divider()
st.subheader("📊 Serial Portfolio Analysis")
bench_start = st.date_input("Backtest Start", date(2023, 1, 1))

@st.cache_data(ttl=86400)
def run_serial_analysis_v23(start_date, monthly_inv_krw, rate):
    ...
    def simulate_benchmark(annual_rate):
        ...
        for d in data.index:
            if d in rebal_dates: balance += monthly_inv_krw
...
bench_res = run_serial_analysis_v23(bench_start, monthly_cash, usd_krw_live)

if not bench_res.empty:
    group = st.radio("Display Group", ["Full (100%)", "QQQ/SGOV Mix", "QLD/SGOV Mix"], horizontal=True)
    mapping = {"Full (100%)": ['QLD 전액', 'QQQ 전액', 'SPYM 전액'], "QQQ/SGOV Mix": ['QQQ/SGOV B&H 7:3', 'QQQ/SGOV SHANNON 8:2'], "QLD/SGOV Mix": ['QLD/SGOV B&H 7:3', 'QLD/SGOV SHANNON 8:2']}
    
    baseline_styles = {'월 급여 순수량': dict(color='#888888', dash='dot', width=2), '연 4% 복리': dict(color='#FFA500', dash='dash', width=2), '연 8% 복리': dict(color='#FF4500', dash='dash', width=2)}
    ...
    fig_bench.update_layout(height=500, template="plotly_dark", yaxis_title="Total Asset (KRW)", xaxis=dict(range=[bench_res.index.min(), bench_res.index.max()]), yaxis=dict(range=[0, global_max * 1.05]), hovermode="x unified")
    st.plotly_chart(fig_bench, use_container_width=True)

# 8. Trade Log (BUY/SELL 선택적 입력기)
st.divider()
st.subheader("📁 Trade Log Viewer")
st.caption("BUY 선택 시 매도가(Sell_Price) 및 실현차익은 비활성화됩니다.")

edited_trade_log = st.data_editor(trade_log_df, num_rows="dynamic", use_container_width=True,
    column_config={"Date": st.column_config.DateColumn("Date", format="YYYY-MM-DD", min_value=date(2000, 1, 1), max_value=date(2050, 12, 31)), 
...
edited_trade_log['Realized_Profit'] = edited_trade_log.apply(lambda x: ((x['Sell_Price'] - x['Buy_Price']) * x['Quantity'] - x['Fees']) * (usd_krw_live if x['Currency'] == 'USD' else 1.0) if x['Action'] in ["SELL", "EXIT_LEGACY"] else 0.0, axis=1)
...
st.caption("© 2026 SHABAL v2.3 | Engineered by Modulus2512")
